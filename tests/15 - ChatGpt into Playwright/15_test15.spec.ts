...
test("Gera um número aleatório de 3 dígitos via ChatGPT e valida a exibição na página", async ({
  page,
}) => {
  ...
  const response = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "Você é um gerador de números aleatórios.",
      },
      {
        role: "user",
        content: "Por favor, gere um número aleatório de 3 dígitos.",
      },
    ],
  });
